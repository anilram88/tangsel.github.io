...
OrgChart.OFFLINE_MESS = "The evaluation version requires intermet connection! <br /><a style='color: #039BE5;' target='_blank' href='https://balkangraph.com/OrgChartJS/Docs/Evaluation'>See more</a>", OrgChart.SEARCH_PLACEHOLDER = "Search", OrgChart.A4w = 595, OrgChart.A4h = 842, OrgChart.A3w = 842, OrgChart.A3h = 1191, OrgChart.A2w = 1191, OrgChart.A2h = 1684, OrgChart.A1w = 1684, OrgChart.A1h = 2384, OrgChart.MIXED_LAYOUT_ALL_NODES = !0, OrgChart.CLINK_CURVE = 1, OrgChart.SEARCH_RESULT_LIMIT = 10, OrgChart.REMOVE_GROUP_IF_HAS_ONE_NODE = !0, "undefined" != typeof module && (module.exports = OrgChart), OrgChart.input = function(t, e, r, i, a, o) {
    null == i && (i = !1);
...
OrgChart.icon = {}, OrgChart.icon.png = function(t, e, r) {
    ...
    for (c = 0; c < s.length; c++) {
        ...
        if (H) {
            ...
            (o[k.id] = k).isAssistant = !0, (x = r[k.id]) && (o[k.id].state = x.state, o[k.id].state == OrgChart.COLLAPSE && m.push(k.id)), OrgChart.server._initStates(k, o, t, a, e, m);
            var _ = [];
            for (b = H.children.length - 1; 0 <= b; b--) {
                (U = H.children[b]).isAssistant ? (U.parent = null, H.children.splice(b, 1), H.childrenIds.splice(b, 1), _.splice(0, 0, U.id)) : (U.pid = k.id, (U.parent = k).children.unshift(U), k.childrenIds.unshift(U.id), H.children.splice(b, 1))
            }
            if (_.length % 2) {
                ...
                OrgChart.templates[I] = {
                    ...
                    nodeMenuButton: "",
                    size: S
                }, (q = new OrgChart.node(H.id + "_transperant", A.pid, [], I)).isAssistant = !0, (x = r[(o[q.id] = q).id]) && (o[q.id].state = x.state, o[q.id].state == OrgChart.COLLAPSE && -1 == m.indexOf(q.id) && m.push(q.id)), _.splice(_.length - 1, 0, q.id), OrgChart.server._initStates(q, o, t, a, e, m)
            }
            var N = 1;
            for (b = _.length - 1; 0 <= b; b--) {
                if (b % 2 && b != _.length - 1)(V = new OrgChart.node(H.id + "_split_" + N, null, [], "split")).isAssistant = !0, o[V.id] = V, _.splice(b, 0, V.id), N++;
    ...
    for (c = 0; c < m.length; c++) {
        if (H = (v = o[m[c]]).parent)
            for (b = H.children.length - 1; 0 <= b; b--) H.children[b] == v && (-1 == H.collapsedChildrenIds.indexOf(v.id) && H.collapsedChildrenIds.push(v.id), H.children.splice(b, 1), "split" == v.templateName || "split" == H.templateName || (v.parent = null))
    }
                ...
                if (H = v.parent) {
                    ...
                    for (b = H.children.length - 1; 0 <= b; b--) {
                        if ((U = H.children[b]) != v && 0 == U.childrenIds.length && !U.isAssistant && "split" != U.templateName) {
                            ...
                            v.children[R] = U, v.layout = OrgChart.mixed
                        }
                    }
                }
        }
    if (a.layout == OrgChart.tree || a.layout == OrgChart.treeRightOffset || a.layout == OrgChart.treeLeftOffset) {
        var X = [];
        for (c = 0; c < n.length; c++) {
            if (0 == (v = o[n[c]]).childrenIds.length && !v.isAssistant && "split" != v.templateName)(H = v.parent) && -1 == X.indexOf(H.id) && X.push(H.id)
        }
        for (c = 0; c < X.length; c++) {
            H = o[X[c]], k = new OrgChart.node(H.id + "_split_0", H.id, [], "split");
            ...
            for (b = H.children.length - 1; 0 <= b; b--) {
                if (OrgChart.server._mixedLayoutAllNodes(H))
                    if (0 == (U = H.children[b]).childrenIds.length && !U.isAssistant && "split" != U.templateName && (U.parent && "split" != U.parent.templateName || U.parent.isAssistant)) {
                        if ((U = H.children[b]).parent = null, U.layout = OrgChart.tree, H.children.splice(b, 1), H.childrenIds.splice(b, 1), a.layout == OrgChart.treeRightOffset && Y.splice(0, 0, U.id), a.layout == OrgChart.treeLeftOffset || a.layout == OrgChart.treeRightOffset) {
                            var q;
                            ...
                            OrgChart.templates[I] = {
                                ...
                                nodeMenuButton: "",
                                size: [0, 0]
                            }, (q = new OrgChart.node(U.id + "_transperant", null, [], I)).layout = OrgChart.tree, o[q.id] = q, Y.splice(0, 0, q.id)
                        }
                        a.layout != OrgChart.treeRightOffset && Y.splice(0, 0, U.id)
                    }
            }
            for (N = 1, b = Y.length - 1; 0 <= b; b--) {
                var V;
                if (b % 2 && b != Y.length - 1)(V = new OrgChart.node(H.id + "_split_" + N, null, [], "split")).layout = OrgChart.tree, o[V.id] = V, Y.splice(b, 0, V.id), N++;
                else b % 2 && Y.splice(b, 0, k.id)
            }
            for (b = 0; b < Y.length; b += 3) {
                M = null;
                0 == b && (M = H);
                E = o[Y[b]], L = o[Y[b + 1]], B = o[Y[b + 2]];
                0 != b && (M = o[Y[b - 3]]), 0 == b || L || (M = o[Y[b - 2]]), E.parent = M, E.pid = M.id, M.children.push(E), H.childrenIds.push(E.id), L && (0 != b && (M = o[Y[b - 2]]), L.parent = M, L.pid = M.id, M.children.push(L), H.childrenIds.push(L.id)), B && (0 != b && (M = o[Y[b - 1]]), B.parent = M, B.pid = M.id, M.children.push(B), H.childrenIds.push(B.id))
            }
        }
    }
    return {
    ...
    if (C.top = l - i.padding, C.left = n - i.padding, C.bottom = o + i.padding - e / x, C.right = a + i.padding - t / x, C.maxX = a, C.minX = n, C.maxY = o, C.minY = l, null == s && i.align == OrgChart.CENTER) {
        ...
        if (b - 2 * i.padding >= o - l) switch (k = (o + l) / 2 - b / 2, i.orientation) {
            case OrgChart.orientation.bottom:
            case OrgChart.orientation.bottom_left:
                k = (l - o) / 2 - b / 2
        } else switch ((k = -(b / 2 - (o - l) / 2)) > -i.padding && (k = -i.padding), i.orientation) {
            case OrgChart.orientation.bottom:
            case OrgChart.orientation.bottom_left:
        ...
        switch (i.orientation) {
            case OrgChart.orientation.top:
                w = h.x - y / 2 + OrgChart.server._getNodeWidth(h, i) / 2, k = -i.padding;
                break;
            case OrgChart.orientation.bottom:
                w = h.x - y / 2 + OrgChart.server._getNodeWidth(h, i) / 2, k = i.padding - b;
                break;
            case OrgChart.orientation.left:
                w = -i.padding, k = h.y - b / 2 + OrgChart.server._getNodeWidth(h, i) / 2;
                break;
            case OrgChart.orientation.right:
                w = i.padding - y, k = h.y - b / 2 + OrgChart.server._getNodeWidth(h, i) / 2;
                break;
            case OrgChart.orientation.top_left:
    ...
    if (d == OrgChart.action.insert || d == OrgChart.action.expandCollapse || d == OrgChart.action.update || d == OrgChart.action.centerNode) {
        var _ = null;
        if (d == OrgChart.action.insert && c && null != c.insertedNodeId && null != c.insertedNodeId)(_ = g[c.insertedNodeId]) || (_ = g[p[c.insertedNodeId]]);
        else if (d == OrgChart.action.update && c && null != c.visId && null != c.visId)(_ = g[c.visId]) || (_ = g[p[c.visId]]);
        else if (d == OrgChart.action.expandCollapse && c && null != c.id && null != c.id)(_ = g[c.id]) || (_ = g[p[c.id]]), _ = OrgChart.server._getMostDeepChild(_, g);
            ...
            _ && (_ || (_ = g[p[c.id]]), _ = OrgChart.server._getMostDeepChild(_, g))
        }
        if (_) switch (i.orientation) {
            case OrgChart.orientation.top:
            case OrgChart.orientation.top_left:
    ...
    for (var r, i, a, o, n, l = "", s = 0; s < this.config.clinks.length; s++) {
        ...
        if ((d.isChildOfGroup || e.res.visibleNodes[d.id]) && ((!d.isChildOfGroup || e.res.visibleNodes[d._groupParentNodeId]) && (c.isChildOfGroup || e.res.visibleNodes[c.id]) && (!c.isChildOfGroup || e.res.visibleNodes[c._groupParentNodeId]))) {
            ...
            switch (this.config.orientation) {
                case OrgChart.orientation.top:
                case OrgChart.orientation.top_left:
                    x <= y ? (k = 1, O.x = x + d.w / 10, v.x = y - c.w / 10) : (k = -1, O.x = x - d.w / 10, v.x = y + c.w / 10), p == u ? (k = 1, O.y = p, v.y = u) : u < p ? (O.y = p, v.y = u + c.h) : (O.y = p + d.h, v.y = u);
                    break;
                case OrgChart.orientation.bottom:
                case OrgChart.orientation.bottom_left:
                    O.x = x <= y ? (k = -1, x + d.w / 10) : (k = 1, x - d.w / 10), v.x = y + c.w / 10, p == u ? (k = -1, O.y = p + d.h, v.y = u + c.h) : u < p ? (O.y = p, v.y = u + c.h) : (O.y = p + d.h, v.y = u);
                    break;
                case OrgChart.orientation.left:
                case OrgChart.orientation.left_top:
                    O.y = b <= w ? (k = -1, b + d.h / 5) : (k = 1, b - d.h / 5), v.y = w + c.h / 5, g == f ? (k = -1, O.x = g, v.x = f) : f < g ? (O.x = g, v.x = f + c.w) : (O.x = g + d.w, v.x = f);
                    break;
                case OrgChart.orientation.right:
                case OrgChart.orientation.right_top:
                    O.y = b <= w ? (k = 1, b + d.h / 5) : (k = -1, b - d.h / 5), v.y = w + c.h / 5, g == f ? (k = 1, O.x = g + d.w, v.x = f + c.w) : f < g ? (O.x = g, v.x = f + c.w) : (O.x = g + d.w, v.x = f)
            }
            var _ = I(O, v, k),
                A = h.template;
            A || (A = "orange");
            m = OrgChart.clinkTemplates[A];
            l += '<g c-link-from="{from}" c-link-to="{to}">'.replace("{from}", d.id).replace("{to}", c.id) + m.link.replaceAll("{d}", "M{a.x},{a.y} C{a.x},{a.y} {c.x},{c.y} {b.x},{b.y}").replaceAll("{b.x}", v.x).replaceAll("{b.y}", v.y).replaceAll("{a.x}", O.x).replaceAll("{a.y}", O.y).replaceAll("{c.x}", _.x).replaceAll("{c.y}", _.y);
            var S = (r = O, a = _, void 0, o = ((i = v).x - r.x) / 2 + r.x, n = (i.y - r.y) / 2 + r.y, {
                x: (o - a.x) / 2 + a.x,
                y: (n - a.y) / 2 + a.y
            });
    ...
    function t(t, e, r) {
        ...
        (t.parent && t.parent.layout == OrgChart.mixed || t.parent && t.parent.layout == OrgChart.tree) && (l = k.config.mixedHierarchyNodesSeparation);
        var s = {
        ...
        if (t.level == e.level) switch (a = e, k.config.orientation) {
            case OrgChart.orientation.top:
            case OrgChart.orientation.top_left:
            ...
            for (var h = t; null == a;) {
                var d = !1,
                    ...
                    g = c.leftNeighbor,
                    p = c.rightNeighbor;
                if (c.id == e.id ? a = c : OrgChart._intersects(c, s, k.config) && (s = OrgChart._addPoint(c, i, k.config, s, o), d = !0), c.id != e.id) {
                    for (; g;) {
                        if (g.id == e.id) {
                            a = g;
                            break
                        }
                        OrgChart._intersects(g, s, k.config) && (s = OrgChart._addPoint(g, i, k.config, s, o), d = !0), g = g.leftNeighbor
                    }
                    for (; p;) {
                        if (p.id == e.id) {
                            a = p;
                        ...
                        OrgChart._intersects(p, s, k.config) && (s = OrgChart._addPoint(p, i, k.config, s, o), d = !0), p = p.rightNeighbor
                    }
                }
                if (!d) {
                    var f = i[i.length - 1][0],
                        u = 0;
                    if (c.parent) switch (l = k.config.levelSeparation, c.parent.layout != OrgChart.mixed && c.parent.layout != OrgChart.tree || (l = k.config.mixedHierarchyNodesSeparation), k.config.orientation) {
                        case OrgChart.orientation.top:
                        case OrgChart.orientation.top_left:
            ...
            switch (n = OrgChart.t(a.templateName, S), i.splice(i.length - 1, 1), k.config.orientation) {
                case OrgChart.orientation.top:
                case OrgChart.orientation.top_left:
        ...
        var x = i.join(" ");
        if (A += '<g c-link-from="{from}" c-link-to="{to}">'.replace("{from}", t.id).replace("{to}", e.id) + n.link.replaceAll("{d}", x), N.label) {
